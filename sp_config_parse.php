...
while($line = fgets($config_file)) {
    if(eregi(';;',$line)) {
        ...
        $array = explode("::", $line);
        $k = strtolower($array[0]);
        if(eregi('\.',$line)) {
            $descriptions[$k]['title'] = '';
            $descriptions[$k]['desc'] = str_replace(
                '"',
                '&quot',
                rtrim($array[1])
            );
            ...
            $descriptions[$k]['title'] = str_replace(
                '"',
                '&quot',
                rtrim($array[1])
            );
...
$new_config_text .= "//mod_rewrite Settings\n"
    . '$modrewrite = '
    . rtrim($options['modrewrite'])
    . ";\n\n";
$new_config_text .= "//Cache Settings\n"
    . '$cachethumbs = '
    . rtrim($options['cachethumbs'])
    . ";\n";
$new_config_text .= '$cachefolder = "'
...
echo "<p>Configuration complete! "
    . "You can now delete this file from your server.</p>";
